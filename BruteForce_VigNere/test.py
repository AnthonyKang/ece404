...
array = ["".join(seq) for seq in itertools.product("01", repeat=16)]
#a = '1110001001110101'
#key_bv = BitVector(bitstring=a)
...
for j in range(0,2):
	key_bv = BitVector(bitstring=array[j])
	print len(key_bv)
	...
	#output = str(key_bv) + ':' + outputtext           
	#print output           #(c)
	#print array[j]
	#FILEOUT.write(array[j]+'\n')
	FILEOUT.write(outputtext)

# Write the plaintext to the output file:
#FILEOUT = open(sys.argv[2], 'w')                                #(d)
#FILEOUT.write(output)                                       #(e)
FILEOUT.close()                                                 #(f)
